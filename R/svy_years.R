...
if (manual) {
  ...
  mss_med <- joyn::joyn(
    ...
    fselect(-.joyn) |>
    # remove urban/rural
    fsubset(!(country_code %in% c("CHN") & reporting_level != "national"))
  ...
  ld_ga <- dt_ga |> # before > dt_ga
    # fsubset(1:2) |>
    fselect(country = country_code, year = reporting_year, welfare_type) |>
    as.list()
...
d_med <- d_spl |>
  fselect(country_code, reporting_year, welfare_type, median)
...
d_spr <- ld_spr |>
  ...
  joyn::joyn(d_med, match_type = "m:1", reportvar = FALSE)
