...
if (!fs::file_exists(lup_medians_f) || isTRUE(force)) {
  dt <- pipapi::pip(popshare = .5, fill_gaps = TRUE, lkup = lkup)
  med <- get_vars(
    dt,
    c(
      "country_code",
...
spl <- med2 |>
  fselect(country_code, reporting_level, spl, reporting_year, welfare_type) |>
...
if (manual) {
  ...
  mpl <- pipapi::pip(povline = povlines, lkup = lkup, fill_gaps = TRUE)
}
...
# duplicate CHN
chnur <- spl[country_code == "CHN"][rep(1:.N, each = 2)][,
  g := rowid(reporting_year)
][,
  reporting_level := fifelse(g == 1, "urban", "rural")
][, g := NULL]
